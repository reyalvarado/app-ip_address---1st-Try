...
//Call helper function getIpv4MappedIpv6Address
return getIpv4MappedIpv6Address(firstIpAddress,ipv6Address);
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    // 
//the above needs fixed - Hint - ipv4: firstIpAddress, ipv6: ipv6Address
      
        console.log(`  Response returned from GET request: {"IPv4":"${sampleIpv4s[i]}","IPv6":"${ipv6Address}"`);
